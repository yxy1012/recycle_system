<template>
  <div v-if="show">
    ...
    <el-input v-model="collectorrequire.username" placeholder="请输入用户姓名" style="width: 200px"></el-input>
...
<script>
  export default {
    methods:{
      ...
      findbyrequire() {
        const _this = this
        axios.post('http://localhost:8181/collectorfindalldonate/1/7',this.collectorrequire).then(function (resp) {
          console.log(resp)
      ...
      page(currentPage) {
        const _this = this
        axios.post('http://localhost:8181/collectorfindalldonate/' + currentPage + '/7', _this.collectorrequire).then(function (resp) {
          _this.tableData = resp.data.list
          _this.pageSize = resp.data.pageSize
          _this.total = resp.data.total
        })
